(function ($) {
    ...
    $(document).ready(function () {
        const calculateSlidesToShow_V4 = () => Math.max(Math.floor($('.repocean-slider-main-v4 .repocean-content-wrapper').width() / 305), 1);
        const initializeSlider_V4 = () => {
            const slidesToShow_v4 = calculateSlidesToShow_V4();
            $('.repocean-slider-main-v4 .slider-box-parent').slick({
                dots: false,
                autoplay: true,
                infinite: true,
                autoplaySpeed: 6100,
        ...
        setTimeout(() => {
            $('.repocean-slider-main-v4 .slider-box-inner, .repocean-slider-main-v4 .review-sub-title').show();
            $('.repocean-slider-main-v4 .repocean-content-wrapper').css('visibility', 'visible');
            refreshReadMore();
        }, 1);
        initializeSlider_V4();
        $(window).on('resize', () => {
    ...
    $('.repocean-slider-main-v4').on('click', '.button-content .rep-button a', function (event) {
        ...
        if (isExpanded) {
            ...
            $button.text(repocean_slider_js_v4.read_more);
        } else {
